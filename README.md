...
- **Sync Mode (P2P 同期リスニング & DJ Room)**: サーバーを介さず6桁のルームコードでWebRTC接続し、ミリ秒単位の再生同期、リアルタイムリクエストキュー・投票、DJアナウンス、チャット機能を備えた共同リスニングモード。
- **Help View**: 各種連携設定 (Discord RPC, SSP SSTP, Software Update)、更新履歴、操作マニュアル。
- **Visual Mode (※現行バージョンではオミット中 / 将来のリニューアルに向けて休止中)**: 楽曲の周波数・音圧に連動する大画面シネマティックビジュアライザー (Quantum Orb, Neon Spectrum, Laser Wave, Radial Vortex, Starfield 3D Galaxy)。※過去バージョンに搭載されていましたが、現行バージョンではパフォーマンス最適化および機能刷新のため一時的に非搭載となっており、今後の大型アップデートでリニューアル予定です。
...
WebRTC (PeerJS) 技術を採用した、完全サーバーレスのリアルタイム共同リスニング機能です。
- **6桁ルームコード接続**: ホストが発行した6桁の英数字コードを入力するだけで、NAT越え・暗号化P2Pセッションを即座に確立。専用サーバー契約や複雑なポート開放は不要です。
- **ミリ秒単位の再生同期**: 参加者間のネットワーク往復遅延時間 (RTT) を自動測定・補正し、再生開始・一時停止・シーク位置を高精度に一致させます。
- **リアルタイム・リクエストキュー & Upvote 投票**: ルーム内の参加者がライブラリから自由に楽曲をリクエスト可能。投票数に応じた自動ソート再生に対応。
- **DJ アナウンス & チャット機能**: ホストやDJによる曲紹介・テロップ送信、参加者全員によるリアルタイムテキストチャットで盛り上がることができます。
- **同期権限管理**: ホストによる再生権限集中モードや、全員が自由に選曲できるフリーモードの切り替えに対応。

### 11. ミニプレイヤーモード (Mini Player Mode)
- **省スペース常時最前面表示**: 画面端にコンパクトに配置可能な専用プレイヤー。
- **軽量操作パネル**: 再生/一時停止、曲送り、シークバー、音量、タイトル表示を凝縮。作業中やゲーム中のBGM再生に最適。

### 12. MediaSession & OSメディアコントロール連携
- **OS標準メディアキー完全対応**: キーボードの再生/一時停止、前の曲、次の曲キーによる操作。
- **Bluetoothオーディオ機器連携**: ヘッドセットやワイヤレスイヤホンのハードウェアボタンからの再生・スキップ制御。
- **OS通知・フライアウト連動**: 音量オーバーレイやロック画面での曲名・アーティスト名表示およびコントロール。

### 13. ワンクリック・インプレイス自動アップデート (One-Click In-App Auto Updater)
...
| `F11` | フルスクリーン表示の切り替え |
| `1` 〜 `6` | プレイヤーモード切替 (1: Single, 2: Dual, 3: Multi, 4: FX, 5: Sync, 6: Help) ※Visual Modeは現行非搭載 |
